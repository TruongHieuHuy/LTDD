...
class ApiService {
    ...
    try {
      ...
      if (response.body.isEmpty) {
        if (response.statusCode >= 200 && response.statusCode < 300) {
          ...
          throw ApiException(
            message:
                defaultErrorMessage ?? 'Request failed with empty response',
      ...
      if (e is ApiException)
        rethrow; // If it's already our custom exception, just rethrow
      // Otherwise, wrap it in a generic network error
      throw ApiException(
        message: 'Network error or parsing issue: ${e.toString()}',
      );
    }
  }
  ...
  Future<UserProfile> getCurrentUser() => _request(
    'GetCurrentUser',
    request: () =>
        http.get(Uri.parse('$baseUrl/api/auth/me'), headers: _headers),
  ...
  /// Upload Avatar - Uploads image file to /api/users/avatar
  Future<String> uploadAvatar(String filePath) async {
    try {
      ...
      throw ApiException(
        message: 'Network error while uploading avatar: ${e.toString()}',
      );
    }
  }
  ...
  Future<List<GameStats>> getStats() => _request(
    'GetStats',
    request: () =>
        http.get(Uri.parse('$baseUrl/api/scores/stats'), headers: _headers),
...
extension FriendAPI on ApiService {
  ...
  Future<List<UserProfile>> searchUsers(String query) => _request(
    ...
    onSuccess: (data) => (data['users'] as List)
  ...
  Future<List<FriendData>> getFriends() => _request(
    ...
    onSuccess: (data) => (data['friends'] as List)
...
extension MessageAPI on ApiService {
  ...
  Future<List<MessageData>> getChatHistory({
    ...
    onSuccess: (data) => (data['messages'] as List)
...
extension SudokuAPI on ApiService {
  ...
  Future<Map<String, dynamic>> validateSudoku({
    ...
    request: () => http.post(
      ...
      body: jsonEncode({'currentState': currentState, 'solution': solution}),
    ),
    onSuccess: (data) => data['data'] as Map<String, dynamic>,
...
extension PuzzleAPI on ApiService {
  ...
  Future<PuzzleGame> generatePuzzle({
    ...
    try {
      debugPrint(
        '🎮 Generating puzzle: difficulty=$difficulty, gridSize=$gridSize',
      );
      ...
      final response = await http.post(
        ...
        headers: _headers,
        body: jsonEncode({'difficulty': difficulty, 'gridSize': gridSize}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '🧩 First tile: ${game.tilePaths.isNotEmpty ? game.tilePaths[0] : "none"}',
        );
        return game;
      } else {
  ...
  Future<int> calculatePuzzleScore({
    ...
    try {
      final response = await http.post(
        Uri.parse(
          '${ApiService.baseUrl}/api/puzzle/calculate-score',
        ), // Không có /api
        headers: _headers,
        body: jsonEncode({
          'difficulty': difficulty,
...
extension PostsAPI on ApiService {
  ...
  Future<Map<String, dynamic>> toggleSavePost(String postId) => _request(
    'ToggleSavePost',
    request: () => http.post(
      Uri.parse('${ApiService.baseUrl}/api/posts/$postId/save'),
      headers: _headers,
    ),
    onSuccess: (data) => data as Map<String, dynamic>,
  ...
  Future<List<AchievementData>> getAllAchievements({String? category}) {
    final Map<String, String> queryParams = {};
    if (category != null && category.isNotEmpty) {
      queryParams['category'] = category;
    }
    final uri = Uri.parse(
      '${ApiService.baseUrl}/api/achievements',
...
extension ChallengeAPI on ApiService {
  ...
  Future<Map<String, dynamic>> checkCaroWinner({
    ...
    try {
      final response = await http.post(
        ...
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({'board': board, 'size': size, 'lastMove': lastMove}),
      );
  ...
  Future<Map<String, dynamic>> createChallenge({
    ...
    request: () => http.post(
      ...
      headers: _headers,
      body: jsonEncode({'opponentId': opponentId, 'betAmount': betAmount}),
    ),
  ...
  Future<Map<String, dynamic>> voteForGame({
    ...
    request: () => http.post(
      ...
      headers: _headers,
      body: jsonEncode({'gameNumber': gameNumber, 'gameType': gameType}),
    ),
    onSuccess: (data) => data['challenge'] as Map<String, dynamic>,
  ...
  Future<Map<String, dynamic>> submitChallengeScore({
    ...
    request: () => http.post(
      ...
      headers: _headers,
      body: jsonEncode({'gameNumber': gameNumber, 'score': score}),
    ),
    onSuccess: (data) => data['challenge'] as Map<String, dynamic>,
  ...
  Future<Map<String, dynamic>> getAdminStats() async {
    return _request(
      'Get Admin Stats',
      request: () => http.get(
        Uri.parse('${ApiService.baseUrl}/api/admin/stats'),
        headers: _headers,
      ),
      onSuccess: (data) => data['data'] as Map<String, dynamic>,
  ...
  Future<List<dynamic>> getRecentActivities({int limit = 10}) async {
    return _request(
      ...
      request: () => http.get(
        Uri.parse(
          '${ApiService.baseUrl}/api/admin/recent-activities?limit=$limit',
        ),
        headers: _headers,
      ),
